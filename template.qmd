...
0. What's your research question
1. Develop a hypothesis
2. Build methodology - develop plan (pseudocode) including what model you'd like to use
3. Find or gather a dataset you want to build insights from. 
4. Look at the data/exploratory data analysis
5. Clean data, remove variables you don't need 
6. Choose models you want to compare. 
7. Choose metrics to evaluate
8. Data split into testing and training
9. Build models
  a. Create our model options, what variables to include 
  b. Data preprocessing
  c. Train models on training data
  d. Tune our model - hyperparameters that control what our models do
  e. Evaluate performance 
  f. Select our model
10. Finalize the model aka train it on whole dataset
11. Interpret/visualize/communicate
...
Will need to format age differently

Price and class likely collinear

Passenger ID and name likely not useful

sib_sp # of siblings and spouses. Maybe useful

Survived will need to be dummy variable, not number 

Same with p_class. It should be a factor

Lots of NA in age and sib_sp

Variable to include: class, age?, embark, fare
...
exp_df <- t_df |>
  select('sex', 'survived') |>
  group_by(sex)|>
  summarise(n=n(), count_surv=sum(survived==1), prop_surv = count_survived/n())
  
exp_df <- t_df |>

ggplot(t_df, aes(x=age, fill = survived))+
  geom_histogram()

ggplot(t_df, aes(x=age, y=))

ggplot(exp_df, aes(x=sex, y=prop)) +
  geom_bar()
...
lasso_rec<-recipe(survived~.,data=surv_train_df) %>%
  update_role(passenger_id, new_role = "ID") %>% #doesn't consider pass id, instead gives it a new role
  step_rm(name,age) %>% #don't consider name and age
  step_unknown(all_nominal(),-all_outcomes()) |> # if you see NA, set to unknown
  step_dummy(all_nominal(),-all_outcomes()) |> # Makes sure everything is a dummy variable
  step_zv(all_numeric(),-all_outcomes()) |> # Get rid of all variables with variance of 0 
  step_normalize(all_numeric(),-all_outcomes()) # We need all our numeric data to be normalized
...
log_test<-surv_test_df |>
  mutate(predict(log_fit, new_data = surv_test_df)) |> #binary classification of 0 and 1
  mutate(predict(log_fit,new_data = surv_test_df, type='prob')) # probability that you think they 
...
lasso_test<-surv_test_df |>
  mutate(predict(lasso_fit, new_data = surv_test_df)) |> #binary classification of 0 and 1
  mutate(predict(lasso_fit,new_data = surv_test_df, type='prob')) # probability that you think they 
...
```{r}
yardstick::roc_auc(log_test, survived, .pred_0)

yardstick::roc_auc(lasso_test, truth=survived, .pred_0)
